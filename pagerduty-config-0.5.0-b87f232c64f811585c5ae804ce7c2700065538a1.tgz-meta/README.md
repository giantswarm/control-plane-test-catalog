...
- **Add a user** — find their PagerDuty ID and add to `users:` in `values.yaml`
- **Add a team and on-call schedule** — 2-PR flow: Crossplane creates the schedules, then you fill in the resulting IDs
- **Add an Alertmanager instance** — add the cluster name to `alertmanagerInstances:` and retrieve the generated integration keys
