...
3. On merge to main, Flux picks up the change and applies it to the gazelle cluster
...
3. Add to `teams:` in `values.yaml`:
   ...
   ```
4. Open a PR — the urgent escalation policy automatically includes all team schedules
...
1. Add the cluster name to `integrations.alertmanager.instances` in `values.yaml`
2. Open a PR
...
- `integrations.urgent.allowedDomains` — domains permitted to trigger incidents; others are suppressed
- Schedule targets are computed automatically from all teams and areas — no manual grouping needed
