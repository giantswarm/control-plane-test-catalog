...
The **alloy-gateway** is a secure ingestion gateway that enables customers to send observability data (logs, traces, and metrics) from external sources to Giant Swarm's Observability Platform. It adds support for external systems, SaaS applications, and hybrid infrastructure scenarios on top of giantswarm-managed cluster-managed data collection.
